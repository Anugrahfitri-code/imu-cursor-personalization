...
Version: 1.0
Status: FROZEN FOR M2.3 CONFIRMATION
Device: Samsung Galaxy A17 4G (SM-A175F)
Characterization baseline: 89b6512
Confirmation software baseline: Git commit/tag containing this frozen rule and the deterministic session-quality evaluator.
Date: 2026-09-15
...
Two new independent 10-minute confirmation runs must be performed after the final numerical rule is frozen.
...
The M2.3 qualification reference interval is 10 ms (100 Hz-equivalent). This interval is used only to define the engineering timing budget. It does not freeze the final preprocessing/resampling grid, which remains a later protocol decision.

Raw Android IMU remains preserved at the native observed acquisition rate. No destructive downsampling is performed during qualification. The final analysis/resampling grid remains to be frozen separately before participant data collection.
...
- 1.0 ms is 10 percent of the 10 ms M2.3 qualification reference interval;
- 5.0 ms is one half of the 10 ms M2.3 qualification reference interval;
- therefore accepted mapping uncertainty remains substantially below one 10 ms M2.3 qualification reference interval for the great majority of mapped timestamps.
...
- Handling of duplicate and reordered packets in the final cursor-update pipeline is a Stage 2 implementation/test requirement. It is not claimed as an M2.3 hard gate before that pipeline exists.
...
- 50 ms corresponds to five 10 ms M2.3 qualification reference intervals and is treated as a hard engineering ceiling, not as a claim of imperceptible interaction latency.
...
These numerical limits are project-specific prospective engineering requirements for this experimental system. They are not claimed to be universal HCI, networking, or IMU standards.
...
Numerical rules proposed in v0.4:
- M2.3 qualification reference interval = 10 ms (100 Hz-equivalent; final preprocessing grid not yet frozen);
...
Freeze status:
- numerical rule: FROZEN FOR CONFIRMATION;
- deterministic session-quality evaluator: IMPLEMENTED;
- evaluator-specific tests: 10/10 PASS;
- full Python regression suite: 79/79 PASS;
- retrospective historical 10-minute evaluator audit: PASS, characterization only;
- prospective confirmation run 01: NOT STARTED;
- prospective confirmation run 02: NOT STARTED;
- M2.3 final qualification freeze: PENDING BOTH CONFIRMATION RUNS.




## 13. Cross-artifact identity and consistency

For participant-representative confirmation runs:

- Android metadata session_id must match every Android local IMU row session_id.
- Android metadata record_name must match every Android local and PC UDP record_name.
- PC UDP session_id must match the Android metadata session_id.
- clock-sync session_id in sync_probes.csv must equal the Android record_name exactly.
- clock_model.json probe sent/valid/invalid counts and response rate must exactly match sync_probes.csv.
- sync_probes.csv is a required evidence artifact for deterministic session-quality evaluation.

These are prospective provenance requirements for confirmation runs. Historical characterization runs are not renamed or rewritten merely to satisfy this convention.




## 14. Frozen confirmation-run environment

The two prospective confirmation runs use the following controlled environment:

- device: Samsung SM-A175F;
- Android recording owner: foreground service;
- Android application remains foreground for the full confirmation run;
- phone screen remains ON for the full confirmation run;
- screen-off timeout is configured to 1800000 ms before each run;
- communication uses Wi-Fi only;
- USB tethering is disabled;
- phone and PC use the same intended local Wi-Fi network path;
- PC receiver is started before Android recording;
- current phone IP is derived immediately before clock-sync startup;
- Android IMU UDP target is verified as the PC address on UDP 5005;
- Android clock-sync listener on UDP 5006 is verified before clock-sync startup;
- no source-code, rule, evaluator, or threshold change is permitted between confirmation run 01 and confirmation run 02;
- the same software commit and rule version are used for both runs;
- record_name and clock-sync session identifier must be identical within each run;
- confirmation duration is 600 seconds for the background-probe duration used by sync_client;
- Android recording continues until sync_client completes, after which Android recording is stopped explicitly.

The confirmation environment is intentionally more controlled than the 25-minute engineering stress/soak characterization. It is designed to represent the planned participant-session technical environment rather than worst-case network stress.

## 15. Freeze declaration

Version 1.0 is frozen before collection of the prospective M2.3 confirmation runs.

Historical characterization runs, including historical runs that satisfy all v1.0 numerical limits, do not count toward the two required confirmation runs.

The planned confirmation run identifiers are:

- m2_3_a17_confirm_10min_01
- m2_3_a17_confirm_10min_02

Both runs must satisfy every v1.0 hard gate.

If either run fails, the failed run is retained. Thresholds are not relaxed and a third run is not substituted merely to obtain two passing results.

Any engineering, configuration, evaluator, or rule change after a failed confirmation requires a new rule version and a new prospective confirmation set.
